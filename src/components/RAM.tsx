...
const RAM:React.FC = function(){
    ...
    const padBy:number = numBase === NumberBase.Binary ? 4: 2;
    return (
    <div className=" text-slate-200 w-80 z-20 ">
      ...
      <div className="border p-4 bg-slate-700">
        ...
        {Object.keys(addressContents).map(curAdd=>(
          <div className={`flex gap-5 justify-between p-2 border ${isMemorySelected && Number(curAdd) === AR && "line-active text-bg-color"}`} key={curAdd}>
            <span className="text-2xl">{Number(curAdd).toString(numberBase).toLocaleUpperCase().padStart(padBy, '0')}</span>
            <span className="text-2xl">{addressContents[Number(curAdd)]}</span>
        </div>
        ))}
      </div>
    </div>
